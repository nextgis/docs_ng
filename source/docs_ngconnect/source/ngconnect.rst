...
1. Введите произвольное название аутентификации;
2. Выберите basic-аутентификацию в выпадающем меню ниже;
3. Заполните поля *Пользователь* и *Пароль* данными вашего NextGIS ID;
4. Нажмите **Сохранить**.

Убедитесь, что выбрана нужная конфигурация и нажмите **Сохранить** в окне "Создание подключения" (:numref: `create_connection_pic`).

Нажмите кнопку "ОК". 

В выпадающем списке "Соединения" диалога "Настройки" выбирается подключение, которое станет активным **после закрытия** диалога.
...
**Добавлять слои в QGIS при создании сервиса** - если опция выбрана, то после завершения операций, в ходе которых был создан сервис WFS, WMS он будет автоматически добавлен в QGIS. 
...
**Директория**, по умолчанию это папка, куда установлена программа.

**Длительность хранения кэша* - 1 день, неделя или месяц, или же не сохранять его вообще.
